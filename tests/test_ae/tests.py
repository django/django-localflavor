...
from localflavor.ae.models import UAEEmiratesIDField, UAEPOBoxField as UAEPOBoxModelField, UAEPostalCodeField
from localflavor.ae.validators import (
...
class UAELocalFlavorTests(TestCase):
    ...
    def test_UAEEmiratesIDField(self):
        ...
        valid = {
            ...
            '784-1800-1234567-1': '784-1800-1234567-1',
            '784-2200-1234567-1': '784-2200-1234567-1',
        }
        invalid = {
    ...
    def test_UAEEmirateField(self):
        ...
        self.assertEqual(field.clean('Dubai'), 'DU')
        self.assertEqual(field.clean('dubai'), 'DU')
        self.assertEqual(field.clean('دبي'), 'DU')

        with self.assertRaises(ValidationError):
            field.clean('XX')

    def test_UAEEmirateField_required_validation(self):
        field = forms.UAEEmirateField()
    ...
    def test_UAETaxRegistrationNumberField(self):
        ...
        valid = {
            ...
            '123 456789012345': '123456789012345',
        }
        invalid = {
...
<option value="UQ">Umm Al Quwain</option>
    ...
    def test_model_full_clean_rejects_invalid_tax_number(self):
        place = UAEPlace(name='Test Location', emirate='DU', tax_number='abc456789012345')

        with self.assertRaises(ValidationError) as cm:
            place.full_clean()

        self.assertEqual(cm.exception.message_dict['tax_number'], ['Enter a valid UAE Tax Registration Number (15 digits).'])

    def test_model_validators_are_migration_serializable(self):
        fields = (
            UAEPlace._meta.get_field('emirates_id'),
            UAEPlace._meta.get_field('postal_code'),
            UAEPlace._meta.get_field('po_box'),
            UAEPlace._meta.get_field('tax_number'),
        )

        for field in fields:
            for validator in field.deconstruct()[3].get('validators', []):
                serializer_factory(validator).serialize()

    def test_model_fields_accept_non_list_validators(self):
        for field_class in (UAEEmiratesIDField, UAEPostalCodeField, UAEPOBoxModelField):
            field = field_class(validators=())
...
class UAEValidatorTests(TestCase):
    ...
    def test_emirates_id_validator_accepts_non_birth_year_block(self):
        """Emirates IDs may contain an identifier block that does not match birth year."""
        validator = UAEEmiratesIDValidator()
    ...
    def test_tax_registration_number_validator(self):
        validator = UAETaxRegistrationNumberValidator()

        validator('123456789012345')
        validator('123 456789012345')
